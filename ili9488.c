			     ...
			     struct drm_plane_state *plane_state)
{
	...
	mipi_dbi_command(mipi, ILI9488_PWCTRLB, 0x00, 0xc1, 0x30);
	mipi_dbi_command(mipi, ILI9488_PWRSEQ, 0x64, 0x03, 0x12, 0x81);
	mipi_dbi_command(mipi, ILI9488_DTCTRLA, 0x85, 0x00, 0x78);
	mipi_dbi_command(mipi, ILI9488_PWCTRLA, 0x39, 0x2c, 0x00, 0x34, 0x02);
	mipi_dbi_command(mipi, ILI9488_PUMPCTRL, 0x20);
	mipi_dbi_command(mipi, ILI9488_DTCTRLB, 0x00, 0x00);

	/* Power Control */
	mipi_dbi_command(mipi, ILI9488_PWCTRL1, 0x23);
	mipi_dbi_command(mipi, ILI9488_PWCTRL2, 0x10);
	/* VCOM */
	mipi_dbi_command(mipi, ILI9488_VMCTRL1, 0x3e, 0x28);
	mipi_dbi_command(mipi, ILI9488_VMCTRL2, 0x86);

	/* Memory Access Control */
	mipi_dbi_command(mipi, MIPI_DCS_SET_PIXEL_FORMAT, MIPI_DCS_PIXEL_FMT_16BIT);

	/* Frame Rate */
	mipi_dbi_command(mipi, ILI9488_FRMCTR1, 0x00, 0x1b);

	/* Gamma */
	mipi_dbi_command(mipi, ILI9488_EN3GAM, 0x00);
	mipi_dbi_command(mipi, MIPI_DCS_SET_GAMMA_CURVE, 0x01);
	mipi_dbi_command(mipi, ILI9488_PGAMCTRL,
			 0x0f, 0x31, 0x2b, 0x0c, 0x0e, 0x08, 0x4e, 0xf1,
			 0x37, 0x07, 0x10, 0x03, 0x0e, 0x09, 0x00);
	mipi_dbi_command(mipi, ILI9488_NGAMCTRL,
			 0x00, 0x0e, 0x14, 0x03, 0x11, 0x07, 0x31, 0xc1,
			 0x48, 0x08, 0x0f, 0x0c, 0x31, 0x36, 0x0f);

	/* DDRAM */
	mipi_dbi_command(mipi, ILI9488_ETMOD, 0x07);

	/* Display */
	mipi_dbi_command(mipi, ILI9488_DISCTRL, 0x08, 0x82, 0x27, 0x00);
...
static int ili9488_probe(struct spi_device *spi)
{
	...
	ret = mipi_dbi_init(&spi->dev, mipi, &ili9488_pipe_funcs,
